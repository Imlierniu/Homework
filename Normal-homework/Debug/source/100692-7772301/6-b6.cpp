...
int tj_strcasecmp(const char *s1, const char *s2)
{
	...
	if(s1!=NULL && s2==NULL)
		return 1;	
	for(;*s1==*s2 && *s1 &&*s2||(*s2>='A'&& *s2<='Z' || *s2>='a'&& *s2<='z' )
		&& (*s1>='A'&& *s1<='Z' || *s1>='a'&& *s1<='z' ) && (*s1==*s2+32 || *s1==*s2-32)  ;s1++,s2++)
		;
    return *s1-*s2;
}
...
int tj_strcasencmp(const char *s1, const char *s2, const int len)
{
	...
	for(;*s1==*s2 && *s1 &&*s2&& ++t!=len||(*s2>='A'&& *s2<='Z' || *s2>='a'&& *s2<='z' )
		&& (*s1>='A'&& *s1<='Z' || *s1>='a'&& *s1<='z' ) && (*s1==*s2+32 || *s1==*s2-32);s1++,s2++)
		;
    return *s1-*s2;
}
...
int tj_strrchr(const char *str, const char ch)
{
 	if(str==NULL)
		return NULL;
	int i;
	for( i=tj_strlen(str)+1,str+=tj_strlen(str);i>=1;str--,i--)
		if(*str==ch)
			return i;
    return 0;
}
...
int tj_strrstr(const char *str, const char *substr)
{
	if(str==NULL)
		return NULL;
	int i,a=tj_strlen(str);
	for(i=0,str+=a-1;i<=a-1;str--,i++)
		if(tj_strncmp(str, substr,tj_strlen(substr))==0)
			return a-i;
	return 0;
}
