...
char *tj_strncpy(char *s1, const char *s2, const int len)
{
	...
	if(s1==NULL) ;
		...
		for(i=0,p=s1;i<len&&*s2>0;i++) *p=*s2,p++,s2++;
		}
    return s1;
}
...
int tj_strcmp(const char *s1, const char *s2)
{
	...
	else if(s2==NULL&&s1!=NULL) n=1;
	else if(s2==NULL&&s1==NULL) n=0;
	else{
		for(;*s1==*s2&&*s1>0;) s1++,s2++;
		if(*s1!=*s2&&*s1>=0&&*s2>=0) n=*s1-*s2;
		else n=0;
		}
    return n;
}
...
int tj_strcasecmp(const char *s1, const char *s2)
{
	...
	if(s1==NULL&&s2!=NULL) n=-1;
		...
		for(;(*s1==*s2&&*s1>0)||(*s1>=65&&*s1<=90&&*s2>=97&&*s2<=122&&*s2==*s1+32)||(*s2>=65&&*s2<=90&&*s1>=97&&*s1<=122&&*s2==*s1-32);) s1++,s2++;
		if(*s1!=*s2&&*s1>=0&&*s2>=0) n=*s1-*s2;
		else n=0;
		}
    return n;
...
int tj_strncmp(const char *s1, const char *s2, const int len)
{
	...
	if(s1==NULL&&s2!=NULL) n=-1;
		...
		if(*s1!=*s2&&*s1>=0&&*s2>=0&&i<len) n=*s1-*s2;
		else n=0;
		}
    return n;
}
...
int tj_strcasencmp(const char *s1, const char *s2, const int len)
{
	...
	if(s1==NULL&&s2!=NULL) n=-1;
		...
		for(i=0;(*s1==*s2&&*s1>0)||(*s1>=65&&*s1<=90&&*s2>=97&&*s2<=122&&*s2==*s1+32)||(*s2>=65&&*s2<=90&&*s1>=97&&*s1<=122&&*s2==*s1-32)&&i<len;i++) s1++,s2++;
		if(*s1!=*s2&&*s1>=0&&*s2>=0&&i<len) n=*s1-*s2;
		else n=0;
		}
    return n;
...
int tj_strstr(const char *str, const char *substr)
{
	...
	if(str==NULL) i=0;
		...
		for(i=1,p=(char *)str;*p>0;p++,i++){
			if(tj_strncmp(p,substr,tj_strlen(substr))==0) break;
			}
		if(*p<=0) i=0;
		}
    return i;
...
int tj_strrchr(const char *str, const char ch)
{
	...
	if(str==NULL) i=0;
		...
		for(i=1,p=(char *)str+tj_strlen(str)-1;*p>0;p--,i++){
			if(*p==ch) break;
			}
		if(*p>0) i=tj_strlen(str)-i+1;
		else if(*p<=0) i=0;
		}
    return i;
}
...
int tj_strrstr(const char *str, const char *substr)
{
	...
	int i;
	if(str==NULL) i=0;
	else if(tj_strlen(substr)>tj_strlen(str)) i=0;
	else{
		for(i=1,p=(char *)str+tj_strlen(str)-1;*p>0;p--,i++){
			if(tj_strncmp(p-tj_strlen(substr)+1,substr,tj_strlen(substr))==0) break;
			}
		if(*p>0) i=tj_strlen(str)-i-tj_strlen(substr)+2;
		else if(*p<=0) i=0;
		}
    return i;
}
...
char *tj_strrev(char *str)
{
	...
	if(str==NULL) ;
		...
		for(j=0,p=str,q=str+tj_strlen(str)-1;*p>0&&j<tj_strlen(str);p++,q--,j=j+2){
			i=*p-*q;
			*p=*q,*q=*q+i;
			}
		}
