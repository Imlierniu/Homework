...
int tj_strncmp(const char *s1, const char *s2, const int len)
{
	...
	for(p=s1,q=s2;(*q!='\0'||*p!=0)&&p<s1+len&&q<s2+len;p++,q++)
	if(*p-*q)
		return(*p-*q);
...
int tj_strstr(const char *str, const char *substr)
{
	...
	for(;*p!='\0';p++)
	{
		if(tj_strncmp(p,q,tj_strlen(q))==0)
			return p-str+1;
	}
		return 0;
...
int tj_strrchr(const char *str, const char ch)
{
	...
	if(str==NULL)
		return 0;
	for(p=tj_strlen(str)+str;p-str>=0;p--)
	{
		if(*p==ch)
			return p-str+1;
	}
...
char *tj_strrev(char *str)
{
	...
	if(str==NULL)
		return 0;
	for(q=str+tj_strlen(str)-1,p=str;p<str-1+tj_strlen(str)/2;p++,q--)
	{
		k=*q;
		*q=*p;
		*p=k;	
